...
with conn.cursor() as cur:
    ...
    while True:
        ...
        send_results_sql = 'insert into logit_tbl (user_id, task_hash, results) values ({}, "{}", \'{}\')'.format(
            user_id, task_hash, logit_rst_json)
        try:
            cur.execute(send_results_sql)
            print('[ok][Ego logit] Logit results are sent to logit_tbl')
        except Exception as e:
        ...
        try:
            ...
            conn.commit()
            print('[ok][Ego logit] The solved task is removed')
        except Exception as e:
        ...
        t1 = time.time()
        print(print_str1)
        print('Ego logit task solving time: {:4.4f} seconds\n\n'.format(t1-t0))
