...
with conn.cursor() as cur:
    if sid == 'all':
        sql = 'select ans_id, user_id, experiment_id, answer from ans_tbl'
    else:
        sql = 'select ans_tbl.ans_id, ans_tbl.user_id, ans_tbl.experiment_id, ans_tbl.answer ' + \
            'from ans_tbl join user_tbl where ans_tbl.user_id=user_tbl.user_id ' + \
            'and user_tbl.sid="{}"'.format(sid) + ' order by ans_tbl.ans_id'
    try:
        # print('\n', sql, '\n')
        cur.execute(sql)
        data = cur.fetchall()
        ...
        print('Python Error: unable to fetch choice data from mysql')
        print(e)
conn.close()
...
logit_param = {var:param for var, param in zip(logit_rst['var'], logit_rst['para'])}
print(logit_param)
...
# pylogit_estimate()
t4 = time.time()
print('\n\n\ntime: import={}, database={}, clogit={}, pylogit={}'.format(t1-t0, t2-t1, t3-t2, t4-t3))